...
function openVinoMenu() {
    ...
    var currentPath = windowLocationArr[windowLocationArr.length - 1].replace(
    ...
    function init() {
        ...
        if (mainNav) {
            ...
            // If "nav-path" exists OR activeMenu returns an empty string,
            ...
            if ($("#top #nav-path").length > 0 || activeMenu === "") {
                activeMenu = "API REFERENCES";
                $activeMenu =
                    $("a:contains('API REFERENCES')", $secondaryMenuContent)
                        .parents("ul > li")
                        .last()
                        .find("ul")
                        .first() || null;
            ...
            renderLeftMenu($activeMenu);
            renderContentsMenus();

            $("#container").css({
                marginTop: document.getElementById("top").offsetHeight + 30
            });
        }
    }
    ...
    function scrollToElement(el) {
        ...
        if (["H2", "H3"].includes(el.parentElement.nodeName)) {
            el = el.parentElement;
        } else {
    ...
    function renderMainMenu(menu) {
        var filteredMenu = menu.map(function () {
            $("ul", this).remove();
            $("a", this)
                .removeClass("has-submenu")
                .removeAttr("aria-expanded aria-haspopup aria-controls");
            if ($("a", this).text() === activeMenu) {
                $("a", this).addClass("active");
            }
            return this;
        });

        // Replace default menu with filteredMenu
        $("#main-nav ul").html(filteredMenu);
    }
    ...
    function renderLeftMenu(menu) {
        ...
        function handleAccordion() {
            ...
            $this.parent("li").toggleClass("accordion-opened");
            $list.toggle();
        }
        ...
        if (menu) {
            ...
            $listItemsWithChildren
                .addClass("has-children accordion")
                .prepend('<span class="accordion-trigger"></span>');
        ...
        if ($("a", $leftNav).length > 0) {
            ...
            var activeListItemStack = $("a.active", $leftNav).parents(
                "ul.main-menu li"
            );
            var activeListItem = $("a.active", $leftNav).parents(
                "ul.main-menu li"
            )[0];
            ...
            if (activeListItemStack.length > 0 && activeListItem) {
                ...
                // Scroll activ list item to top of menu on page load
                $leftNav[0].scrollTop = activeListItem.offsetTop - 60;
            }
...
function getCurrentVersion() {
    ...
    var index = window.location.href.indexOf(protocol);
    var link = window.location.href.slice(index + protocol.length);
    return link.split('/')[1] !== "index.html" && link.split('/')[1] !== "404.html" && link.split('/')[1] !== "" ? link.split('/')[1] : versions[0].version;
}
